...
urlpatterns = [
    ...
    path('post/(?<pk>\d+)/remove/',
         views.PostDeleteView.as_view(), name='post_remove'),
    path('drafts/', views.DraftListView.as_view(), name='post_draft_list'),
    path('post/(?<pk>\d+)/comment/', views.add_comment_to_post,
         name='add_comment_to_post'),
    path('comment/(?<pk>\d+)/approve/',
         views.comment_approve, name='comment_approve'),
    path('comment/(?<pk>\d+)/remove/',
         views.comment_remove, name='comments_remove'),
